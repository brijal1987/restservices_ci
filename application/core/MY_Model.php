...
class MY_Model extends CI_Model {
    ...
    public function update_data($table_name = "", $data = array(), $where = "", $is_delete = "", $batch = FALSE) {
        ...
        if (!empty($table_name) && !empty($data)) {
            ...
            if ($batch) {
                ...
                if (empty($is_delete)) {
                    $data["modified_on"] = CURRENT_DATETIME;
                    $data["modified_by"] = $this->session->id;
                } else {
                    $data["deleted_on"] = CURRENT_DATETIME;
                    $data["deleted_by"] = $this->session->id;
                }
                if (is_array($where) && !empty($where)) {
                    $this->db->where($where);
                } elseif ($where != "") {
    ...
    public function _having_query($column_having = array(), $search_string = "") {
        ...
        foreach ($column_having as $i => $item) { // loop column
            if (isset($post_columns[$i]) && $post_columns[$i]["searchable"] == true && $post_columns[$i]["search"]["value"] != "") {
                $andhaving .= " AND " . $item . " like '%" . $this->db->escape_like_str(trim($post_columns[$i]["search"]["value"])) . "%'";
            }
        }
        $andhaving .= ")";
        $orhaving = "";
        if (!empty($search_string)) {
            foreach ($column_having as $i => $item) { // loop column
                if (!empty($item)) {
                    ...
                    $orhaving .= $item . " like '%" . $this->db->escape_like_str(trim($search_string)) . "%'";
                }
            }
    ...
    public function get_products($data) {
        $this->db->select("id, product_category, yield_type_id, product_name, yield_percentage");
        $this->db->from("core_product_yields");
        if (isset($data['product_category']) && $data['product_category'] != '') {
            $this->db->where("product_category", $data['product_category']);
        }
        if (isset($data['fish_specie_id']) && $data['fish_specie_id'] != false) {
            $this->db->where_in("fish_specie_id", $data['fish_specie_id']);
        }
    ...
    public function get_emailtemplate_by_type($type) {
        $retarray = array();
        $this->db->select("*");
        $this->db->from("core_email_templates");
        ...
        if ($query->num_rows() > 0) {
            $row = $query->row_array();
            $retarray['id'] = $row["id"];
            $retarray['notification_type'] = stripslashes($row["notification_type"]);
            $retarray['name'] = stripslashes($row["name"]);
            $retarray['subject'] = stripslashes($row["subject"]);
            $retarray['content'] = stripslashes($row["content"]);
        }
